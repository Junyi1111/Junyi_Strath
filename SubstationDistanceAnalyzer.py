...
class SubstationDistanceAnalyzer:
    ...
    def visualize_network(self, num_substations=12):
        fig, ax = plt.subplots(figsize=(10, 10))
        self.hv_cable.plot(ax=ax, color='lightgray', linewidth=1, zorder=1, label="HV Cable")
        ax.scatter(self.sub_point.x, self.sub_point.y, color='red', s=150, edgecolor='k', zorder=5,
                   label='Primary Substation', marker='o')
        ...
        for idx, row in self.closest_substations.iterrows():
            ...
            try:
                shortest_path_nodes = nx.shortest_path(self.G, source=self.nearest_source_node,
                                                       target=nearest_target_node, weight='length')
                ax.scatter(row['geometry'].x, row['geometry'].y, color='blue', s=100,
                           label='Secondary Substation' if not secondary_label_added else "", zorder=4, marker='o')
                ...
                continue
        ax.set_title("18 Battlefield Road substation")
        ax.legend(loc="upper left")
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.text(self.sub_point.x + 1200, self.sub_point.y - 1600, "|--- 100m ---|", ha='center', va='center',
                fontsize=9, fontweight='bold', color='black')
        ax.plot([self.sub_point.x + 1100, self.sub_point.x + 1300], [self.sub_point.y - 1700, self.sub_point.y - 1700],
                color='black', linewidth=2)
        ax.set_xlim(self.sub_point.x - 2000, self.sub_point.x + 2000)
        ax.set_ylim(self.sub_point.y - 2000, self.sub_point.y + 2000)
        plt.show()
